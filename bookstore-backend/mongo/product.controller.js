...
module.exports = {
  ...
  getNewProductAuthor,
  getDecrease,
  getAscending,getDecreaseAuthor,getAscendingAuthor,getDecreasePublish,getAscendingPublish, getNewProductPublish,
  getSale,
  getByCategory,
  getByPublish,
  getByAuthor,
 getPaginatedProductsByPublisherSorted,getPaginatedProductsByAuthorSorted,getPaginatedProductsByCategorySorted,getPaginatedAndSortedProducts
};
...
async function getPaginatedProductsByCategorySorted(category, pageNumber, limit, sortBy) {
  try {
      ...
      const categoryObjectId = mongoose.Types.ObjectId.isValid(category) ? new mongoose.Types.ObjectId(category) : null;
      ...
      const totalProducts = await productModel.countDocuments({ "category.categoryId": categoryObjectId });
...
async function getPaginatedProductsByAuthorSorted(author, pageNumber, limit, sortBy) {
  try {
    const totalProducts = await productModel.countDocuments({ "author.authorId": new mongoose.Types.ObjectId(author) });
...
async function getPaginatedProductsByPublisherSorted(publisher, pageNumber, limit, sortBy) {
  try {
    const totalProducts = await productModel.countDocuments({ "publisher.publisherId": new mongoose.Types.ObjectId(publisher) });
